...
toolPrepareEUstatisticalPocketbook <- function(x) {
  
  dataTable <- magpie2dt(x)
  mapfile <- system.file("extdata", "mappingEUpocketbookToEDGET.csv", package = "mrtransport", mustWork = TRUE)
  mapping <- fread(mapfile, skip = 0)
  mappedData <- merge(dataTable, mapping, by = "transportMode")
